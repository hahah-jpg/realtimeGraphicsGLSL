...
float vnoise21(vec2 p) { // 2次元値ノイズ
    ...
    for (int j = 0; j < 2; j++) {
        for (int i = 0; i < 2; i++) {
            v[i+2*j] = hash21(n + vec2(i, j)); // マスの4頂点のハッシュ値
        }
    }
    vec2 f = fract(p);
    if (channel == 1) { // 中央：エルミート補完
        f = f * f * (3.0 -2.0 * f);
    }
    return mix(mix(v[0], v[1], f[0]), mix(v[2], v[3], f[0]), f[1]); // 左：双曲線補完
}
float vnoise31(vec3 p) {
    ...
    vec3 f = fract(p);
    f = f * f * (3.0 -2.0 * f); // エルミート補完
    float[2] w;
    for (int i = 0; i < 2; i++) {
        w[i] = mix(mix(v[4*i], v[4*i+1], f[0]), mix(v[4*i+2], v[4*i+3], f[0]), f[1]); // 底面と上面での補完
    }
    return mix(w[0], w[1], f[2]); // 高さに関する補完
}
void main() {
